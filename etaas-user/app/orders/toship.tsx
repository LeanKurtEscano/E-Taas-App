...
const ToShipScreen = () => {
    ...
    if (order.status === 'confirmed') {
      return (
        <View className="bg-pink-100 px-3 py-1.5 rounded-full flex-row items-center">
          <CheckCircle size={14} color="#ec4899" strokeWidth={2.5} />
  ...
  const getStatusNote = (order: Order) => {
    if (order.status === 'confirmed') {
      return (
        <View className="bg-pink-50 px-4 py-3 rounded-xl border border-pink-100 mt-3">
          <View className="flex-row items-start">
            <View className="w-8 h-8 rounded-full bg-pink-500 items-center justify-center mr-3 mt-0.5">
            ...
            <View className="flex-1">
              ...
              <Text className="text-sm text-pink-700">
                The seller is preparing to ship your item.
              </Text>
            </View>
          </View>
        </View>
      ...
      <View className="bg-pink-50 px-4 py-3 rounded-xl border border-pink-100 mt-3">
        <View className="flex-row items-start">
          ...
          <View className="flex-1">
            ...
            <Text className="text-sm text-pink-700 mb-3">
              Your order has been shipped! You can track it using the link below.
            </Text>
            {order.shippingLink && (
              <TouchableOpacity
                onPress={() => handleOpenTrackingLink(order.shippingLink!)}
                className="bg-pink-500 px-4 py-2.5 rounded-lg flex-row items-center justify-center"
                activeOpacity={0.8}
                style={{
                  shadowColor: '#ec4899',
      ...
      >
        <View className="px-5 pb-4">
          <View className="flex-row items-center">
            ...
            <View className="flex-1">
              <Text className="text-2xl font-bold text-gray-900">To Ship</Text>
              <Text className="text-sm text-gray-500 mt-0.5">
                {orders.length} {orders.length === 1 ? 'order' : 'orders'} to be shipped
              </Text>
            </View>
          </View>
        </View>
        ...
        {orders.length === 0 ? (
          <View className="flex-1 justify-center items-center py-20 px-6">
            ...
            <Text className="text-gray-500 text-center">
              Orders that are being prepared or shipped will appear here
            </Text>
          </View>
        ) : (
          <View className="px-4 py-4">
            {orders.map((order) => (
              ...
              >
                ...
                {/* Status Note */}
                <View className="px-5 py-4">{getStatusNote(order)}</View>
              </View>
            ))}
          </View>
        )}
